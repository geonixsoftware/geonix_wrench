...
from extraction import extract_jobcard
...
from transcription import preload_model, transcribe_audio
...
app = FastAPI(title="Geonix Wrench API")
...
async def process_audio(
    ...
    try:
        ...
        try:
            ...
            extraction = extract_jobcard(transcript)
...
def get_jobcard_pdf(
    ...
    jobcard = get_jobcard(jobcard_id, resolve_owner_scope(user))
    if jobcard is None:
        raise HTTPException(status_code=404, detail="Job card not found")

    effective_labor_rate = jobcard["labor_rate"] if jobcard["labor_rate"] is not None else x_labor_rate
    pdf_bytes = generate_jobcard_pdf(jobcard, currency=x_currency, labor_rate=effective_labor_rate)
    return Response(
        content=pdf_bytes,
...
def create_organization(
    ...
    try:
        org = database.create_organization(user["id"], payload.name, payload.seat_limit)
...
def get_billing_status_route(user: dict = Depends(get_current_user)) -> BillingStatusResponse:
    return BillingStatusResponse(**billing.get_billing_status(user))
